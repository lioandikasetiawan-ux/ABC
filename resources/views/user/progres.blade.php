...
@section('content')
    <div class="w-full px-6 py-4">
        ...
        <div class="mb-8">
            ...
            <p class="text-sm text-slate-500 ml-4">Status verifikasi dokumen secara real-time untuk setiap paket pekerjaan yang Anda kelola</p>
        </div>
                ...
                <table class="w-full text-left min-w-[1000px]">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-100">
                            ...
                            <th class="px-4 py-4 text-xs font-semibold text-slate-500 text-center">Berkas Ditolak</th>
                    ...
                    <tbody class="divide-y divide-slate-50">
                        ...
                        @foreach($pakets as $p)
                            {{-- LOGIKA: Hanya tampilkan jika user sudah mulai upload (step_selesai > 0) --}}
                            @if($p->step_selesai > 0)
                                @php 
                                    $hasData = true;
                                    $uploadPercent = round(($p->step_selesai / 11) * 100);
                                    $denominatorVerifikasi = $p->step_selesai > 0 ? $p->step_selesai : 1;
                                    $progressPercent = round(($p->step_verifikasi / 11) * 100);

                                    $statusConfig = [
                                        'selesai' => ['bg-emerald-100 text-emerald-700', 'CheckCircle'],
                                        'terpenuhi' => ['bg-emerald-50 text-emerald-600 border border-emerald-200', 'Check'],
                                ...
                                <tr class="hover:bg-slate-50/80 transition-colors group">
                                    ...
                                    <td class="px-4 py-5 text-center">
                                        <div class="inline-flex flex-col items-center">
                                            <span class="text-lg font-bold text-slate-700">
                                                {{ $p->step_selesai }}<span class="text-sm font-medium text-slate-400">/11</span>
                                            </span>
                                            <div class="w-16 h-1 bg-slate-100 rounded-full mt-1 overflow-hidden">
                                                <div class="h-full bg-indigo-400 rounded-full" style="width: {{ $uploadPercent }}%"></div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    {{-- Berkas Ditolak --}}
                                    <td class="px-4 py-5 text-center">
                                        <span class="text-lg font-bold {{ $p->step_ditolak > 0 ? 'text-red-600' : 'text-slate-700' }}">
                                            {{ $p->step_ditolak }}<span class="text-sm font-medium text-slate-400">/{{ $p->step_selesai }}</span>
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-4 py-5 text-center">
                                        <div class="inline-flex flex-col items-center">
                                            <span class="text-lg font-bold {{ $p->step_verifikasi == 11 ? 'text-emerald-600' : 'text-indigo-600' }}">
                                                {{ $p->step_verifikasi }}<span class="text-sm font-medium text-slate-400">/{{ $p->step_selesai }}</span>
                                            </span>
                                            <div class="w-16 h-1 bg-slate-100 rounded-full mt-1 overflow-hidden">
                                                @php $verifVisualPercent = ($p->step_verifikasi / $denominatorVerifikasi) * 100; @endphp
                                                <div class="h-full {{ $p->step_verifikasi == 11 ? 'bg-emerald-500' : 'bg-indigo-500' }} rounded-full"
                                                    style="width: {{ $verifVisualPercent }}%"></div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    {{-- Status --}}
                                    <td class="px-4 py-5 text-center">
                                        <span class="inline-flex items-center gap-1.5 px-3 py-1.5 {{ $statusClass }} text-xs font-medium rounded-lg">
                                            @if($statusIcon == 'CheckCircle')
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                            @elseif($statusIcon == 'Check')
                                    ...
                                    <td class="px-6 py-5">
                                        <div class="flex items-center gap-3">
                                            <div class="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                                                <div class="h-full rounded-full transition-all duration-500 {{ $p->step_verifikasi == 11 ? 'bg-emerald-500' : 'bg-indigo-500' }}"
                                                    style="width: {{ $progressPercent }}%">
                                                </div>
                                            </div>
                                            <span class="text-xs font-semibold text-slate-600 min-w-[40px]">{{ $progressPercent }}%</span>
                                        </div>
                                    </td>
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center gap-3">
                                        <svg class="w-12 h-12 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                                            <path d="M9 12h6m-6 4h6m2-10H7a2 2 0 00-2 2v14l4-4h10a2 2 0 002-2V6a2 2 0 00-2-2z"></path>
                                        </svg>
                                        <p class="text-slate-400 font-medium">Belum ada paket yang sedang diproses</p>
                                        <a href="{{ route('user.wizard') }}" class="text-indigo-600 text-sm hover:underline font-semibold">Pilih Paket Sekarang &rarr;</a>
                                    </div>
                                </td>
                            </tr>
                        @endif
