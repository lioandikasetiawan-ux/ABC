        ...
        <div class="max-w-2xl mx-auto">
                ...
                <div class="p-8">
                    ...
                    <form x-data="{ paketId: '' }" :action="'/wizard/paket/' + paketId + '/step/1'" method="GET"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <select x-model="paketId" required
                                    ...
                                    @foreach ($pakets as $paket)
                                        ...
                                        @endphp
                                        <option value="{{ $paket->id }}" {{ $isLocked ? 'disabled' : '' }}
                            ...
                            {{-- Progress Indicator for Selected Paket (Optional) --}}
                            {{-- <template x-if="paketId">
                                <div class="mt-3 text-center">
                                    @php
                                        $selectedPaket = $pakets->firstWhere('id', $paket->id ?? null);
                                    @endphp
                                    <div class="inline-flex items-center gap-2 px-3 py-1.5 bg-slate-100 rounded-lg">
                                        ...
                                        <div class="w-20 h-1.5 bg-slate-200 rounded-full overflow-hidden">
                                            <div class="h-full bg-indigo-600 rounded-full"
                                                :style="'width: ' + ({{ $selectedPaket->progres_persen ?? 0 }} || 0) + '%'">
                                            </div>
                                        </div>
                                        <span class="text-xs font-medium text-slate-700"
                                            x-text="({{ $selectedPaket->progres_persen ?? 0 }} || 0) + '%'"></span>
                                    </div>
                                </div>
                            </template> --}}
                        </div>
                        ...
                        {{-- Warning Message for Locked Paket --}}
            ...
            <div class="mt-6 text-center">
                <p class="text-xs text-slate-400">
                    Total {{ $pakets->count() }} paket tersedia
                </p>
            </div>
        </div>
    </div>
