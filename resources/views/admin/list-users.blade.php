...
@section('content')
    <div class="max-w-6xl mx-auto">
        <div class="mb-6">
            ...
            <p class="text-gray-500">Menampilkan seluruh satker yang mengunggah pada <strong>Step
                    {{ $stepNumber }}</strong></p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-50">
                            ...
                            <tr class="hover:bg-blue-50/30 transition">
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        <div
                                            class="w-8 h-8 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center font-bold text-xs mr-3">
                                ...
                                <td class="px-6 py-4">
                                    @if ($sub->status == 'approved')
                                        <span
                                            class="px-2 py-1 bg-green-100 text-green-700 text-[10px] font-bold uppercase rounded-full">Disetujui</span>
                                    @elseif($sub->status == 'rejected')
                                        <span
                                            class="px-2 py-1 bg-red-100 text-red-700 text-[10px] font-bold uppercase rounded-full">Ditolak</span>
                                    @else
                                        <span
                                            class="px-2 py-1 bg-yellow-100 text-yellow-700 text-[10px] font-bold uppercase rounded-full">Pending</span>
                                    @endif
                                </td>
                                    ...
                                    <div class="flex justify-center gap-2" x-data="{ openReject: false }">
                                        ...
                                        <template x-if="openReject">
                                            <div
                                                ...
                                                <div
                                                    class="bg-white p-6 rounded-2xl w-96 text-left shadow-2xl border border-gray-100">
                                                    <h3 class="text-lg font-bold text-gray-800 mb-2">Tolak Submission?</h3>
                                                    <p class="text-sm text-gray-500 mb-4">Berikan alasan penolakan agar user
                                                        dapat memperbaiki dokumen.</p>

                                                    <form action="{{ route('admin.verify.submit', $sub->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        <input type="hidden" name="status" value="rejected">
                                                        <textarea name="catatan_admin"
                                                            class="w-full border-gray-200 rounded-xl text-sm focus:ring-red-500 focus:border-red-500 mb-4" rows="3"
                                                            placeholder="Contoh: File tidak terbaca atau data tidak sesuai." required></textarea>

                                                        <div class="flex justify-end gap-2">
                                                            <button type="button" @click="openReject = false"
                                                                class="px-4 py-2 text-sm text-gray-500 hover:text-gray-700">Batal</button>
                                                            <button type="submit"
                                                                class="px-4 py-2 text-sm bg-red-600 text-white rounded-lg hover:bg-red-700 transition">Kirim
                                                                Penolakan</button>
                                                        </div>
                                                    </form>
                                                </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-10 text-center text-gray-400 italic">Belum ada user yang
                                mengunggah dokumen pada tahap ini.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
